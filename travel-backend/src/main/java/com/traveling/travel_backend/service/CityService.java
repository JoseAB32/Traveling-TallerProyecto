...
public class CityService {
    ...
    public List<CityResponseDTO> getAllCities() {
        ...
        logRepository.save(new LogEntity(
                AppConstants.LOG_CITIES,
                AppConstants.LOG_INFO,
                "Petición recibida: GET /api/cities",
                null
        ));
        ...
        logRepository.save(new LogEntity(
                AppConstants.LOG_CITIES,
                AppConstants.LOG_INFO,
                "Ciudades encontradas: " + cities.size() + " y devueltas correctamente.",
                null
        ));
    ...
    public CityResponseDTO getCityById(Long id) {
        ...
        logRepository.save(new LogEntity(
                AppConstants.LOG_CITIES,
                AppConstants.LOG_INFO,
                "Solicitando información de la ciudad con ID: " + id + " - GET /api/cities/" + id,
                null
        ));

        City city = cityRepository.findById(id)
                .orElseThrow(() -> {
                    logger.warn("{} [{}] Ciudad con ID {} no encontrada",
                            AppConstants.PREFIX_CITY, AppConstants.LOG_CITIES, id);
                    logRepository.save(new LogEntity(
                            AppConstants.LOG_CITIES,
                            AppConstants.LOG_WARN,
                            "Ciudad con ID " + id + " no encontrada - GET /api/cities/" + id,
                            null
                    ));
                    return new ResourceNotFoundException("Ciudad no encontrada con ID: " + id);
                });

        logger.debug("{} [{}] Ciudad encontrada: {}", AppConstants.PREFIX_CITY, AppConstants.LOG_CITIES, city);
